...
export class AuthService {
   ...
   async register(data: RegisterDTO): Promise<AuthResponse> {
      ...
      if (Array.isArray(existingEmployee) && existingEmployee.length > 0) {
         const error = new ConflictError("Email address is already registered");
         (error as any).field = "email";
         throw error;
      }
      ...
      try {
         ...
         const userDetails = await this.userRepository.findUserWithEmployeeDetails(
            userId,
            connection
         );
         if (!userDetails) {
         ...
         const tokenPayload: TokenPayload = {
            ...
            employee_name: `${userDetails.employee?.first_name || ""} ${
               userDetails.employee?.last_name || ""
            }`.trim(),
   ...
   async login(data: LoginRequestDTO): Promise<AuthResponse> {
      ...
      const tokenPayload: TokenPayload = {
         ...
         company_name: userWithDetails.company?.company_name,
         branch_id: userWithDetails.employee?.branch_id,
         branch_name: userWithDetails.branch?.branch_name,
         employee_name: `${userWithDetails.employee?.first_name || ""} ${
            userWithDetails.employee?.last_name || ""
         }`.trim(),
   ...
   async refreshToken(refreshTokenString: string): Promise<AuthResponse> {
      try {
         ...
         const userWithDetails = await this.userRepository.findUserWithEmployeeDetails(
            payload.userId
         );
         if (!userWithDetails) {
   ...
   private async getOrCreateAdminRole(connection: any): Promise<number> {
      ...
      if (existingRoles.length > 0) {
         const roleId = existingRoles[0].role_id;
   ...
   private async createEmployee(
      data: RegisterDTO & { company_id: number },
      connection: any
   ): Promise<number> {
      const employeeData = {
         uuid: uuidv4(),
