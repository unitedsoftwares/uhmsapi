...
export abstract class BaseRepository<T extends BaseEntity> {
   ...
   async create(data: any, createdBy: string, connection?: PoolConnection): Promise<T> {
      const conn = connection || this.pool;
      const uuid = uuidv4();
      const now = new Date();
   ...
   async hardDelete(identifier: string | number, connection?: PoolConnection): Promise<boolean> {
      const conn = connection || this.pool;
      const [result] = await conn.execute<ResultSetHeader>(
