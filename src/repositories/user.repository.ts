...
export class UserRepository extends BaseRepository<User> {
   ...
   async createUserWithEmployee(
      ...
      userData: Omit<
         User,
         "user_id" | "employee_id" | keyof import("../models/base.model").BaseEntity
      >,
      createdBy: string,
      connection?: PoolConnection
      ...
      try {
         ...
         const [employeeResult] = await conn.execute<ResultSetHeader>(
            `INSERT INTO employees (${employeeColumns.join(
               ", "
   ...
   async findById(userId: number, connection?: PoolConnection): Promise<User | null> {
      const conn = connection || this.pool;
      const [rows] = await conn.execute<RowDataPacket[]>("SELECT * FROM users WHERE user_id = ?", [
         userId,
      ]);
      return rows.length > 0 ? (rows[0] as User) : null;
   }
   ...
   async findUserWithEmployeeDetails(
      userId: number,
      connection?: PoolConnection
   ): Promise<any | null> {
      const conn = connection || this.pool;
